...
export class AuthService {
  ...
  async login(dto: LoginDto) {
    ...
    const existing = await this.refreshTokenRepo.findOne({ where: { userId: user.id, deviceId } });
    if (existing) {
      existing.tokenHash = tokenHash;
      existing.expiresAt = expiresAt;
      await this.refreshTokenRepo.save(existing);
    } else {
      await this.refreshTokenRepo.save({ userId: user.id, deviceId, tokenHash, expiresAt });
    }
